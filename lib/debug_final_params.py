...
# 获取处理后的参数
final_params = request.get_params(base_params.copy())
uri = '/aweme/v1/web/user/profile/other/'
a_bogus = request.get_sign(uri, final_params)
...
logger.info("\n=== 关键参数检查 ===")
logger.info(f"webid: {final_params.get('webid', 'MISSING')}")
logger.info(f"uifid: {final_params.get('uifid', 'MISSING')}")
logger.info(f"msToken: {final_params.get('msToken', 'MISSING')[:50]}...")
a_bogus_val = final_params.get('a_bogus', 'MISSING')
if a_bogus_val != 'MISSING':
    logger.info(f"a_bogus: {str(a_bogus_val)[:50]}...")
else:
    logger.info(f"a_bogus: {a_bogus_val}")
logger.info(f"verifyFp: {final_params.get('verifyFp', 'MISSING')}")

# 检查headers
logger.info("\n=== Headers检查 ===")
headers = request.HEADERS.copy()
if '/user/profile/other/' in uri:
    headers['Referer'] = f'https://www.douyin.com/user/{final_params.get("sec_user_id", "")}'
